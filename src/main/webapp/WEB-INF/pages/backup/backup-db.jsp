        ...
        <tr class="<c:if test="${archive.status == 2}">success</c:if> <c:if test="${archive.status == 3}">error</c:if>">
            ...
            <td>
                <c:if test="${archive.status == 2}">
                    <a href="<spring:url value="/archive/download/database/${archive.id}" />" class="mdl-button mdl-js-button mdl-button--fab mdl-js-ripple-effect mdl-button--mini-fab">
                        <i class="material-icons">file_download</i>
                    </a>
                </c:if>
                <c:if test="${archive.status == 3}">
                    <a href="<spring:url value="/archive/error/database/${archive.id}" />" class="mdl-button mdl-js-button mdl-button--fab mdl-js-ripple-effect mdl-button--mini-fab">
